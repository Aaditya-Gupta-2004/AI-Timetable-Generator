...
function HeroCanvas() {
  ...
  useEffect(() => {
    ...
    const cards = POS.map(([x, y, z], i) => {
      ...
      m.userData = { baseY: y, speed: 0.4 + Math.random() * 0.6, phase: Math.random() * Math.PI * 2 };
      scene.add(m);
      return m;
    });
    ...
    frame.position.set(0.3, 0.6, -1.5);
    frame.rotation.set(0.12, -0.18, 0);
    scene.add(frame);
    ...
    const tick = () => {
      t += 0.012;
      cards.forEach((c) => {
        c.position.y = c.userData.baseY + Math.sin(t * c.userData.speed + c.userData.phase) * 0.18;
        c.rotation.z = Math.sin(t * 0.3 + c.userData.phase) * 0.06;
      });
...
function StatsCanvas() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      t += 0.012;
      rings.forEach((r, i) => {
        r.rotation.z = t * 0.4 + i * 0.8;
        r.rotation.x = Math.PI / 2.5 + Math.sin(t * 0.3 + i) * 0.2;
      });
      icos.forEach((m, i) => {
...
function FeaturesCanvas() {
  ...
  useEffect(() => {
    ...
    const positions = [[-1.2, 0.8, 0], [1.0, 1.0, -0.5], [0, -0.8, 0.3], [-0.9, -1.0, -0.2], [1.2, -0.5, 0.4], [0.1, 0.1, -1]];
    const shapes = positions.map(([x, y, z], i) => {
      ...
      m.position.set(x, y, z);
      m.userData = { rx: Math.random() * 0.02 - 0.01, ry: Math.random() * 0.02 - 0.01, phase: Math.random() * Math.PI * 2, baseY: y };
      scene.add(m);
      return m;
    });
    ...
    const tick = () => {
      ...
      shapes.forEach((s) => {
        ...
        s.position.y = s.userData.baseY + Math.sin(t + s.userData.phase) * 0.15;
      });
      renderer.render(scene, camera);
      raf = requestAnimationFrame(tick);
    };
...
function CtaCanvas() {
  ...
  useEffect(() => {
    ...
    const orbiters = [0xf43f5e, 0xf59e0b, 0x6366f1].map((col, i) => {
      ...
      scene.add(mesh);
      return { mesh, radius: 2.1, speed: 0.4 + i * 0.2, offset: (i / 3) * Math.PI * 2 };
    });
    ...
    const tick = () => {
      ...
      outerRing.rotation.x = Math.PI / 3 + Math.sin(t * 0.1) * 0.1;
      ring2.rotation.x = t * 0.15;
      ring2.rotation.z = -t * 0.25;
      orbiters.forEach(({ mesh, radius, speed, offset }) => {
        mesh.position.x = Math.cos(t * speed + offset) * radius;
        mesh.position.y = Math.sin(t * speed + offset) * radius * 0.5;
        mesh.position.z = Math.sin(t * speed * 0.7 + offset) * 0.5;
      });
      renderer.render(scene, camera);
      raf = requestAnimationFrame(tick);
    };
...
function FooterCanvas() {
  ...
  useEffect(() => {
    ...
    const waveLines = [0x0d9488, 0xf43f5e, 0xf59e0b].map((col, li) => {
      const pts = [];
      for (let i = 0; i <= 80; i += 1) pts.push(new THREE.Vector3(-5 + i * 0.125, 0, 0));
    ...
    const tick = () => {
      ...
      waveLines.forEach((line) => {
        ...
        for (let i = 0; i <= 80; i += 1) pos.setY(i, Math.sin(i * 0.18 + t + line.userData.offset) * line.userData.amplitude);
  ...
  return (
    <>
      ...
      <nav style={{
        ...
        padding: "0 24px 0 32px", height: 84,
      ...
      <section style={{ minHeight: "100vh", position: "relative", overflow: "hidden", display: "flex", alignItems: "center", background: `radial-gradient(ellipse 80% 60% at 60% 40%,rgba(13,148,136,.18) 0%,transparent 70%),radial-gradient(ellipse 50% 50% at 20% 70%,rgba(244,63,94,.14) 0%,transparent 60%),linear-gradient(160deg,#0a1628 0%,#060e1c 100%)` }}>
        ...
        <div className="hero-content">
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center", animation: "fadeUp .8s .5s both" }}>
            <a href="/generate" className="btn-primary btn-glow">Generate Timetable →</a>
      ...
      <footer style={{ borderTop: "1px solid rgba(255,255,255,.07)", position: "relative", overflow: "hidden" }}>
        ...
        <div className="footer-inner">
          ...
          >
            ...
            />
            <span style={{ fontSize: 13, color: "rgba(255,255,255,.4)", fontFamily: "'DM Sans',sans-serif" }}>Timetable Generator © 2026</span>
          </div>
