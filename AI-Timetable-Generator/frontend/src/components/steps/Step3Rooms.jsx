...
export default function Step3Rooms({
  rooms, roomNum, setRoomNum, roomType, setRoomType,
  roomError, setRoomError, addRoom, removeRoom,
  yearBranches,
  roomAssignMode, setRoomAssignMode,
  ybRoomConfig, toggleRoomInPool,
  setActiveTab,
}) {
  ...
  return (
    <>
            ...
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              {classroomPool.map(r => (
                <div key={r.id} style={S.roomRow}>
                  <span style={{ fontWeight: 600, fontFamily: "monospace" }}>{r.number}</span>
              ...
              <div key={yb.id} style={{ marginBottom: 16, border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden" }}>
                ...
                {mode === "manual" && (
                  <div style={{ padding: "14px 16px", display: "flex", gap: 16, flexWrap: "wrap" }}>
                    {[
                      { key: "theory",   label: "📚 Theory",   pool: classroomPool },
                      { key: "elective", label: "🎯 Elective", pool: classroomPool },
                      { key: "lab",      label: "🔬 Lab",      pool: labPool },
                    ].map(({ key, label, pool }) => (
                      <div key={key} style={{ flex: 1, minWidth: 160 }}>
                        ...
                        {pool.map(r => (
                          <label key={r.id} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, fontSize: 12, cursor: "pointer" }}>
                            <input
                              type="checkbox"
                              checked={(config[key] || []).includes(r.number)}
                              onChange={() => toggleRoomInPool(yb.id, key, r.number)}
                            />
                ...
                {mode === "auto" && (
                  <div style={{ padding: "10px 16px", fontSize: 12, color: "#888" }}>
                    All {classroomPool.length} classroom(s) → theory &amp; electives &middot; All {labPool.length} lab(s) → core lab rotation
                  </div>
                )}
              </div>
            );
      ...
      {/* Back → Subjects (tab 2) | Next → Teachers (tab 4) */}
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <button className="card-btn btn-ghost" onClick={() => setActiveTab(2)}>← Back</button>
        <button className="card-btn btn-blue" style={{ padding: "10px 28px" }} onClick={() => setActiveTab(4)}>Next: Teachers →</button>
      </div>
    </>
  );
}